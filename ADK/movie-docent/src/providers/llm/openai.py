"""
OpenAI LLM 어댑터 — A/B 비교용 (Phase 4 stretch).

Gemini 패턴 그대로 재사용. langchain-openai 의존성은 pyproject에서 주석 처리돼 있으니
실제 사용 전에 주석 해제 + `pip install -e .` 필요.

사용:
    .env에서 LLM_PROVIDER=openai로 전환
"""
...
class OpenAILLM(LLMProvider):
    """`ChatOpenAI` 위에 LLMProvider 인터페이스를 씌운 얇은 래퍼."""
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "OpenAI 사용하려면 pyproject의 'langchain-openai' 주석 해제 후 "
